...
# Define a function to reduce a list of directions by removing unnecessary opposite pairs
def dir_reduc(arr):
    # Dictionary that maps each direction to its opposite
    oppDict = {
        'NORTH': 'SOUTH',
        'SOUTH': 'NORTH',
        'EAST': 'WEST',
    ...
    # Iterate over the array to find adjacent pairs of opposite directions
    for i in range(len(arr) - 1):
        # If the next direction is the opposite of the current one
        if arr[i + 1] == oppDict[arr[i]]:
            # Remove both directions (they cancel each other out)
            del arr[i], arr[i]  # Removes arr[i] twice (shifts left after first deletion)

            # Recursively call the function to re-check the updated list from the beginning
            return dir_reduc(arr)
    
    # If no more reductions are possible, return the reduced list
    return arr